...
public class Style {
    ...
    public static String EDGE_END = "</edge>\n";
    ...
    public static String CONT_MAP_END = "</continuousMapping>\n</visualProperty>\n";
    public static String NODE_SIZE_MIN = "\"50.0\"";
    public static String NODE_SIZE_MAX = "\"750.0\"";

    public static String EDGE_WIDTH_START = "<visualProperty name=\"EDGE_WIDTH\" default=\"2.0\">\n<continuousMapping attributeType=\"long\" attributeName=\"edge_degree\">\n";
    public static String EDGE_SIZE_MIN = "\"2.0\"";
    public static String EDGE_SIZE_MAX = "\"30.0\"";
	...
	try {
		...
		if (Long.parseLong(temp[4]) > edge_max) edge_max = Long.parseLong(temp[4]);
	    ...
	    bw.write(NETWORK);
	    bw.write(EDGE);
	    bw.write(EDGE_WIDTH_START);
	    String temp = POINT_START + EDGE_SIZE_MIN + GREATER + EDGE_SIZE_MIN + EQUAL + EDGE_SIZE_MIN + ATT + "\"1.0\"" + POINT_END; 
	    bw.write(temp);
	    String e_max = '"' + Long.toString(edge_max) + '"';
	    temp = POINT_START + EDGE_SIZE_MAX + GREATER + EDGE_SIZE_MAX + EQUAL + EDGE_SIZE_MAX + ATT + e_max + POINT_END + CONT_MAP_END; 
	    bw.write(temp);
	    bw.write(EDGE_END);
	    ...
	    temp = POINT_START + NODE_SIZE_MIN + GREATER + NODE_SIZE_MIN + EQUAL + NODE_SIZE_MIN + ATT + "\"1.0\"" + POINT_END; 
	    bw.write(temp);
	    String deg_max = '"' + Long.toString(max) + '"';
	    temp = POINT_START + NODE_SIZE_MAX + GREATER + NODE_SIZE_MAX + EQUAL + NODE_SIZE_MAX + ATT + deg_max + POINT_END + CONT_MAP_END; 
	    bw.write(temp);
	    bw.write(NODE_END);
	    bw.write(XML_END);
	    fileReader.close();
